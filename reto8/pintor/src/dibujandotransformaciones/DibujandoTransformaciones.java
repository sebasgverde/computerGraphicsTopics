...
public class DibujandoTransformaciones extends JPanel {
  ...
  public void funcion()
  {
      ...
      mat = new Matriz3(0,0,0,0,0,0,0,0,0);
       ...
       try{
        ...
        int numeroFiguras;
        numeroFiguras = Integer.parseInt(br.readLine());
  ...
  public void paintComponent(Graphics g) {
    ...
    formas.get(0).crearPoligono();
    formas.get(1).crearPoligono();
    
    if(hayConflicto(formas.get(0),formas.get(1)))
    {
        g2d.setColor(Color.GREEN);
        g2d.fillPolygon(formas.get(1).poligono);
        g2d.setColor(Color.BLUE);
        g2d.fillPolygon(formas.get(0).poligono);
    }
        else
    {
  ...
  public boolean hayConflicto(Forma formaA, Forma formaB)
  {
      Vector3f a = formaA.puntos.get(0).cambiarA3F();
      Vector3f b = formaA.puntos.get(1).cambiarA3F();
      Vector3f c = formaA.puntos.get(2).cambiarA3F();
      Plano p = new Plano(a, b, c);
            
      if(formaB.minZ > formaA.maxZ)
          return false;
      else if(!formaA.poligono.getBounds2D().intersects(formaB.poligono.getBounds2D()))
          return false;
      else if(noEstaEnPlano(formaA, formaB))
          return false;
      else if(noEstaEnPlano(formaB, formaA))
          return false;      
      else
      return true;
  }
  
  public boolean noEstaEnPlano(Forma formaA, Forma ab)
  {
      Vector3f a = formaA.puntos.get(0).cambiarA3F();
      Vector3f b = formaA.puntos.get(1).cambiarA3F();
      Vector3f c = formaA.puntos.get(2).cambiarA3F();
      ...
      for(int i=0; i<ab.puntos.size();i++)
      {
           if(p.evaluarPunto(ab.puntos.get(i).cambiarA3F()) <= 0);
            return false;
      }
      return true;
  }
